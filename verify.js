...
test('新版状态顶层集合固定且数据版本已升级', () => {
  ...
  assertEqual(context.store.parkStateV2Version, 'demo-2026-09-16-1', '数据版本');
});
...
test('充值批次状态、依据和时间必须一致', () => {
  ['online', 'offline'].forEach(method => {
    ...
    if (method === 'online') earlyEvidence.rechargeBatches[0].paymentReference = 'PAY-EARLY';
    else earlyEvidence.rechargeBatches[0].voucher = 'voucher-early.pdf';
    assert(!parkState.internals.isValidState(earlyEvidence), method + ' pending 不得提前有支付依据');

    const wrongEvidence = clone(valid);
    if (method === 'online') wrongEvidence.rechargeBatches[0].voucher = 'voucher-wrong.pdf';
    else wrongEvidence.rechargeBatches[0].paymentReference = 'PAY-WRONG';
    assert(!parkState.internals.isValidState(wrongEvidence), method + ' 不得携带另一方式依据');

    const invalidCreatedAt = clone(valid);
    invalidCreatedAt.rechargeBatches[0].createdAt = 'not-a-date';
    assert(!parkState.internals.isValidState(invalidCreatedAt), '批次 createdAt 无效应拒绝');

    const earlyPostedAt = clone(valid);
    earlyPostedAt.rechargeBatches[0].postedAt = new Date().toISOString();
  ...
  invalidPostedAt.rechargeBatches[0].postedAt = 'not-a-date';
...
test('统一 invoiceRecords 支持严格 service_fee_invoice 记录', () => {
  ...
  ['merchantId', 'consumeParkId', 'sourceId', 'amount', 'status', 'createdAt'].forEach(field => {
    ...
    if (field === 'amount') invalid.invoiceRecords[0][field] = 0;
    else if (field === 'createdAt') invalid.invoiceRecords[0][field] = 'not-a-date';
    else invalid.invoiceRecords[0][field] = null;
    assert(!parkState.internals.isValidState(invalid), '缺少或非法 ' + field + ' 的服务费发票应被拒绝');
  });
  const mismatchedPark = clone(valid);
  mismatchedPark.invoiceRecords[0].consumeParkId = 'park-002';
